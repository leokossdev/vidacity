...
**Vidacity iOS** — a native SwiftUI YouTube client that replicates the YouTube mobile app: home feed, search, playback, login, subscriptions, library, history, playlists, comments, likes, downloads. Plus a "Link" tab for downloading from any of ~2,000 sites supported by yt-dlp.
...
**Bundle identifier:** `app.vidacity` (kept to preserve provisioning profiles, Keychain access, and `UserDefaults` keys for existing installs). Also the `os.Logger` subsystem.
...
| Logging | `os.Logger` with subsystem `app.vidacity` |
...
├── Resources/
│   ├── Localizable.xcstrings    # en / es / ru, 174 keys
...
5. Cookies are serialized as a single Cookie-header string and stored in Keychain under `app.vidacity.cookies` via `KeychainHelper`.
...
7. **`SubscriptionRegistry`** persists the user's subscribed channel IDs in `UserDefaults` (`app.vidacity.subscriptions`). Subscribe/unsubscribe optimistically flips this **and** calls the YouTube endpoint. Needed because YouTubeKit's `subscribeStatus` parser doesn't follow `pageHeaderRenderer` entity-key indirection — channel screens were always showing "Subscribe" even on subscribed channels until this cache was added.
...
```swift
private let log = Logger(subsystem: "app.vidacity", category: "PlaybackResolver")
...
- `Localizable.xcstrings` (Apple String Catalog format), 174 keys, fully translated for **en / es / ru**. `sourceLanguage: "en"`.
