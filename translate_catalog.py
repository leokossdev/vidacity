#!/usr/bin/env python3
"""
One-shot localization filler for Vidacity/Localizable.xcstrings.
...
CATALOG = Path(__file__).parent / "Vidacity" / "Vidacity" / "Localizable.xcstrings"
...
T = {
    ...
    "Vidacity is a personal/sideload-only YouTube client. It uses YouTubeKit (cookie-based, no Google API key) plus yt-dlp for downloads. YouTube can change its internal API at any time — please be patient when things break.": {
        "es": "Vidacity es un cliente de YouTube solo para uso personal/sideload. Utiliza YouTubeKit (basado en cookies, sin clave de API de Google) y yt-dlp para las descargas. YouTube puede cambiar su API interna en cualquier momento — ten paciencia cuando algo falle.",
        "ru": "Vidacity — клиент YouTube только для личного использования/sideload. Использует YouTubeKit (на основе cookies, без ключа Google API) и yt-dlp для загрузок. YouTube может изменить свой внутренний API в любой момент — наберитесь терпения, если что-то перестанет работать.",
        "fr": "Vidacity est un client YouTube réservé à l’usage personnel / au sideload. Il utilise YouTubeKit (basé sur les cookies, sans clé d’API Google) et yt-dlp pour les téléchargements. YouTube peut modifier son API interne à tout moment — soyez patient lorsque quelque chose se casse.",
        "de": "Vidacity ist ein YouTube-Client ausschließlich für die persönliche Nutzung bzw. das Sideloading. Er verwendet YouTubeKit (cookie-basiert, ohne Google-API-Schlüssel) sowie yt-dlp für Downloads. YouTube kann seine interne API jederzeit ändern — bitte habe Geduld, wenn etwas nicht funktioniert.",
    },
    "Vidacity uses a `WKWebView` to capture cookies. They are stored only in the Keychain.": {
        "es": "Vidacity usa un `WKWebView` para capturar cookies. Se almacenan solo en el Llavero.",
        "ru": "Vidacity использует `WKWebView` для получения cookies. Они хранятся только в Keychain.",
        "fr": "Vidacity utilise un `WKWebView` pour capturer les cookies. Ils sont stockés uniquement dans le Trousseau.",
        "de": "Vidacity verwendet einen `WKWebView`, um Cookies zu erfassen. Sie werden ausschließlich im Schlüsselbund gespeichert.",
    },
    "Vidacity — see App/RootView.swift": {
        "es": "Vidacity — ver App/RootView.swift",
        "ru": "Vidacity — см. App/RootView.swift",
        "fr": "Vidacity — voir App/RootView.swift",
        "de": "Vidacity — siehe App/RootView.swift",
    },
    ...
    "yt-dlp is the engine that resolves YouTube stream URLs. Vidacity auto-refreshes it every 7 days from the official GitHub release. Tap Update now if a video stops playing — newer versions often fix breakage caused by YouTube's API changes.": {
        "es": "yt-dlp es el motor que resuelve las URLs de stream de YouTube. Vidacity lo actualiza automáticamente cada 7 días desde la release oficial de GitHub. Toca «Actualizar ahora» si un vídeo deja de reproducirse — las versiones nuevas suelen corregir fallos provocados por cambios en la API de YouTube.",
        "ru": "yt-dlp — это движок, который получает URL потоков YouTube. Vidacity автоматически обновляет его раз в 7 дней из официального релиза на GitHub. Нажмите «Обновить сейчас», если видео перестало воспроизводиться — новые версии часто исправляют проблемы, вызванные изменениями API YouTube.",
        "fr": "yt-dlp est le moteur qui résout les URLs de flux YouTube. Vidacity le met à jour automatiquement tous les 7 jours depuis la release officielle sur GitHub. Touchez « Mettre à jour » si une vidéo ne se lit plus — les nouvelles versions corrigent souvent les pannes dues aux changements de l’API YouTube.",
        "de": "yt-dlp ist die Engine, die YouTube-Stream-URLs auflöst. Vidacity aktualisiert sie automatisch alle 7 Tage aus dem offiziellen GitHub-Release. Tippe auf „Jetzt aktualisieren“, wenn ein Video nicht mehr abspielt — neue Versionen beheben oft Brüche durch YouTube-API-Änderungen.",
    },
    "• %@": {"es": "• %@", "ru": "• %@", "fr": "• %@", "de": "• %@"},
    "≈ %@": {"es": "≈ %@", "ru": "≈ %@", "fr": "≈ %@", "de": "≈ %@"},
}
