...
Open `Vidacity.xcodeproj` and drag each of these folders into the project navigator under the existing `Vidacity` group, with **"Create groups"** selected and the **`Vidacity`** target ticked:
...
- Add `app.vidacity.resume-downloads` to `Info.plist` under `BGTaskSchedulerPermittedIdentifiers`.
...
- ⚠️ Tests (§14) are not included in this scaffold. Add `VidacityTests/` mirroring the source structure when starting P0 verification.

## 5. SourceKit diagnostics during scaffolding

Most "cannot find type X in scope" errors visible right now are because the new files are not yet members of the `Vidacity` build target. They resolve after step 1.
