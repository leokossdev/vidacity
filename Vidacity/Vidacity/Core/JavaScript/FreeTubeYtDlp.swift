...
public nonisolated func freetube_yt_dlp(
    ...
    let bridgeLog = AppLog(subsystem: "app.vidacity", category: "VidacityYtDlp")
    ...
    // `_VidacityPop` wraps the package's `Pop` (delegating ffmpeg/ffprobe) and additionally
...
public nonisolated func freetube_yt_dlp_extract_info(url: String) async throws -> PythonObject {
    let bridgeLog = AppLog(subsystem: "app.vidacity", category: "VidacityYtDlp")
    let ytdlpLog = AppLog(subsystem: "app.vidacity", category: "ytdlp")
    ...
    // same `subsystem == app.vidacity` predicate. Detached + `.utility` so the
